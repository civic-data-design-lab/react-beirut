...
export default class App extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    map.current = new mapboxGl.Map({
      ...
      maxBounds: [
        [35.383297650238326, 33.83527318407196],
        [35.629842811007315, 33.928357422091395],
      ],
    });
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    for (const workshop of this.props.workshops) {
      ...
      el.style.backgroundColor =
        this.colorMap[workshop.craft_discipline_category[0]];
      el.style.borderRadius = '50%';
      el.onclick = this.clickMarker;
      el.id = workshop.ID;
      ...
      const indices = craftType.map((craft) => {
        return this.props.filterSearchData['filteredCraftsParent'].indexOf(
          craft
        );
      });
      ...
      if (
        ...
        if (
          this.props.filterSearchData['toggleStatusParent'] &&
          workshop.shop_status !== 'open'
        ) {
          continue;
        }
        ...
        if (
          lookup === '' ||
          (shopName &&
            shopName.slice(0, lookup.length).toUpperCase() ===
              lookup.toUpperCase()) ||
          (shopOrig &&
            shopOrig.slice(0, lookup.length).toUpperCase() ===
              lookup.toUpperCase())
        ) {
          el.lng = lng;
          el.lat = lat;
