...
export default class MapCard extends React.Component {
  ...
  getThumbnails = () => {
    ...
    let thumbNails = this.state.similarWorkshops.map((simWorks) => (
      <img
        key={simWorks.thumb_img_id}
        className={'mapCard-img'}
        src={`/api/images/${simWorks.thumb_img_id}.jpg`}
        alt="img"
      />
    ));
    return thumbNails;
    ...
    if (this.props.type === 'workshop') {
      return (
        <div className={'mapCard'} id={`mapCard${this.props.id}`}>
          ...
          <p>
            ...
            {this.props.workshop.craft_discipline.map((craft) => {
              if (craft.toUpperCase() === 'OTHER') {
                ...
                return (
                  '| ' +
                  craft.charAt(0).toUpperCase() +
                  craft.slice(1).toLowerCase() +
                  ' '
                );
              }
            })}{' '}
          ...
          <MapCardSlider
            children={this.getImages()}
            sliderStyle={this.state.mainSliderStyle}
            getImageData={this.getCaption}
          />
  ...
  render() {
    ...
    return <>{this.props.workshop && this.createMapCardContent()}</>;
  }
}
