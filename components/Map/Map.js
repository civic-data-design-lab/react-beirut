  ...
  componentDidMount() {
    ...
    for (const workshop of this.props.workshops) {
      ...
      if (workshop.craft_discipline_category.length > 1) {
        ...
        firstCraft.style.backgroundColor = `${
          this.colorMap[workshop.craft_discipline_category[0].toLowerCase()]
        }`;
        ...
        secondCraft.style.backgroundColor = `${
          this.colorMap[workshop.craft_discipline_category[1].toLowerCase()]
        }`;
    ...
    for (const archive of this.props.archives) {
      ...
      if (archive.craft_discipline_category.length > 1) {
        ...
        secondCraft.style.pointerEvents = 'none';
        firstCraft.style.backgroundColor = `${
          this.colorMap[archive.craft_discipline_category[0].toLowerCase()]
        }`;
        firstCraft.style.width = `${this.state.markerRadius}px`;
        firstCraft.style.height = `${this.state.markerRadius * 2}px`;
        secondCraft.style.backgroundColor = `${
          this.colorMap[archive.craft_discipline_category[1].toLowerCase()]
        }`;
        secondCraft.style.width = `${this.state.markerRadius}px`;
        secondCraft.style.height = `${this.state.markerRadius * 2}px`;
        el.appendChild(firstCraft);
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (changedZoom || changedCoords) {
      if (this.props.showMapCard === false && this.props.coords) {
        map.current.flyTo({
          center: changedCoords ? this.props.coords : [35.51, 33.893894],
    ...
    if (
      ...
      this.props.showMapCard
    ) {
      if (
        this.props.coords[0] !== 35.51 &&
        this.props.coords[1] !== 33.893894
      ) {
        map.current.flyTo({
          ...
          offset:
            this.state.width < 992
              ? 687 < this.state.width
                ? [0, this.state.height * 0.3]
                : [0, -this.state.height * 0.25]
              : [0, 0],
          bearing: 0,
          speed: 0.5, // make the flying slow
        ...
        if (this.state.activeLayer) {
          ...
          if (!this.props.showMapCard) {
            let filtered = Object.fromEntries(
              Object.entries(this.props.allLayers).filter(
                ([k, v]) => v[0] === this.props.mapLayer
              )
            );
            if (Object.values(filtered)[0][3]) {
              this.props.setMapLayerSettings(
                Object.values(filtered)[0][3],
                Object.values(filtered)[0][4]
              );
            }
          }
          map.current.setLayoutProperty(
    ...
    if (prevState.geoLocateCoords != this.state.geoLocateCoords) {
      if (this.state.geoLocateCoords) {
        ...
        el.style.borderRadius = '50%';
        let marker = new mapboxGl.Marker(el)
          .setLngLat(this.state.geoLocateCoords)
          .addTo(map.current);
    ...
    const sameWorkshopToggle =
      prevProps.toggleWorkshopStatus === this.props.toggleWorkshopStatus;
    const sameArchiveToggle =
    ...
    for (const workshop of this.props.workshops) {
      ...
      if (workshop.year_established == null) {
        ...
        if (
          start <= workshop.year_established &&
          workshop.year_established <= end
        ) {
          withinInterval = true;
        } else {
          withinInterval = false;
      ...
      const noCrafts =
        ((!workshop.craft_discipline_category ||
          workshop.craft_discipline_category.length < 1) &&
          this.props.filteredCrafts &&
          this.props.filteredCrafts.length < 1) ||
        (this.props.filteredCrafts && this.props.filteredCrafts.length === 7);
    ...
    for (const archive of this.props.archives) {
      ...
      if (archive.primary_year) {
        ...
        if (
          start <= archive.primary_decade[0] &&
          archive.primary_decade[0] <= end
        ) {
          withinInterval = true;
        }
      } else {
      ...
      const noCrafts =
        ((!archive.craft_discipline_category ||
          archive.craft_discipline_category.length < 1) &&
          this.props.filteredCrafts &&
          this.props.filteredCrafts.length < 1) ||
        (this.props.filteredCrafts && this.props.filteredCrafts.length === 7);
      if (
    ...
    return (
      <>
        ...
        <div className={'nav-ctr-container'}>
          <button
            className={'nav-ctr-btn zoom-in-btn btn-interactivity'}
          ...
          <button
            className={'nav-ctr-btn zoom-out-btn btn-interactivity'}
          ...
          >
            ...
            >
              <path
                d="M5.03906 0.390625V1.89062H0.90625V0.390625H5.03906Z"
                fill="#471E10"
              />
            </svg>
          </button>
