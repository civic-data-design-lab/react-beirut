...
const _getImageDataFromForm = (data, id) => {
  let imageMeta = null;
  ...
  let imageId;
  if (data.workshop_images.imageData) {
    imageId = `${id}_1`;
    ...
    imageData = {
      img_id: imageId,
      filename: imageName,
      data: data.workshop_images.imageData,
    };
  }
...
const convertWorkshopContributionToSchema = (data) => {
   ...
   */
  let { imageId, imageMeta, imageData } = _getImageDataFromForm(data, newWorkshopId, imageType, location);
