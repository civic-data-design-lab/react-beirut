 ...
 */
const _arrayify_true_false_mapping = (mapping) => {
  const trueValues = [];
  const falseValues = [];
 ...
 */
const _get_location_from_form = (data) => {
  let address = null;
...
const convertWorkshopContributionToSchema = (data) => {
  ...
  // Get the image type
  const { trueValues: imageType } = _arrayify_true_false_mapping(
    data.image_content?.defaultTags
  );

  // Craft disciplines
  // @GatlenCulp - Update this if your craft type implementation changes
  const { trueValues: craftDiscipline } = _arrayify_true_false_mapping(
    data.type_of_craft?.defaultTags
  );
  const { trueValues: craftDisciplineCategory } = _arrayify_true_false_mapping(
  ...
  // Get the location
  const location = _get_location_from_form(data);
...
const get_distance_between_points = (geo1, geo2) => {
  try {
    ...
    const length = turfLength.default(line, { units: 'meters' });
    return length;
  } catch (err) {
    console.error(err);
    return Infinity;
