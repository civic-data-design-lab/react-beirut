...
const _getImageDataFromWorkshopForm = (data, id, imageIndex = 0) => {
  ...
  if (data.images[imageIndex].imageData) {
    ...
    const imageUrl = `http://cddl-beirut.herokuapp.com/api/images/${imageName}`;
...
const convertWorkshopContributionToSchema = (data, formSchema) => {
   ...
   */
  let { imageId, imageMeta, imageData } = _getImageDataFromWorkshopForm(
    data,
    newWorkshopId
  );
  ...
  const workshop = {
    ...
    craft_discipline_category: data[craftPage.fields.craft_category.field_name],
    craft_discipline: data[craftPage.fields.type_of_craft.field_name],
...
const convertArchiveContributionToSchema = (data, formSchema) => {
  ...
  const { imageId, imageMeta, imageData } = _getImageDataFromArchiveForm(
    data,
    newArchiveId
  );
  ...
  const archive = {
    ...
    craft_discipline: data[imagePage.fields.type_of_craft.field_name],
    ...
    reference: {
      name: data[aboutPage.fields.reference_name.field_name],
      ref_type: null,
      type_other: null,
      location: null,
      catalog: null,
