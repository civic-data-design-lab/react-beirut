...
export const saveNewWorkshop = async (data) => {
  ...
  try {
    ...
    if (data.imageMetas?.length > 0) {
      ...
      // INFO: Save the image data for the workshop.
      for (const imgDataOrig of data.imageDataOriginal) {
        ...
        imgDataThumbnail.filename = `${imgData.img_id}_thumbnail.jpeg`;
...
export const saveNewArchive = async (data) => {
  ...
  try {
    ...
    if (data.imageMetas?.length > 0) {
      ...
      for (const imgData of data.imageDataOriginal) {
        // Convert image data to a buffer.
        imgData.data = imgData.data.split(',').pop();
        imgData.data = Buffer.from(imgData.data, 'base64');
        const newImageDataOriginal = new ImageDataOriginal(imgData);
        const res = await newImageDataOriginal.save();
        savedImageDataOriginal.push(res);
      }
    }
  } catch (err) {
    console.error(err);
...
export const getStickerData = async (code) => {
  ...
  if (!sticker) {
    return {
      stickerData: null,
      message: `Sticker with code "${code.split(3, 5)}" not found`,
    };
  }
