...
export default async (req, res) => {
  ...
  Archive.find({}, { craft_discipline: 1, craft_discipline_other: 1 }).exec(
    (err, archives) => {
      for (let archive of archives) {
        ...
        if (archive.craft_discipline_other) {
          if (Array.isArray(archive.craft_discipline_other)) {
            ...
            craftTypes.add(
              archive.craft_discipline_other.charAt(0).toUpperCase() +
                archive.craft_discipline_other.slice(1).toLowerCase()
            );
          }
        }
      }
      Workshop.find(
        {},
        { craft_discipline: 1, craft_discipline_other: 1 }
      ).exec((err, workshops) => {
        for (let workshop of workshops) {
          ...
          if (workshop.craft_discipline_other) {
            if (Array.isArray(workshop.craft_discipline_other)) {
              ...
              craftTypes.add(
                workshop.craft_discipline_other.charAt(0).toUpperCase() +
                  workshop.craft_discipline_other.slice(1).toLowerCase()
              );
            }
          }
        }
