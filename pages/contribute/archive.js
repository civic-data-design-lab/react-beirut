      ...
      custom_reqs: {
        ref_source_req: {
          function: (formData) => {
            ...
            if (
              (formData[fields.type_of_reference.field_name] ==
                'Print source' ||
                formData[fields.type_of_reference.field_name] ==
                  'Electronic source') &&
    ...
    setForm((prevForm) => {
      ...
      if (cookiesEnabled && !localStorageFull) {
        try {
          ...
          setDialog(
            'Because your browser has exceeded its storage, it can no longer ' +
      ...
      try {
        const convertedAS = convertArchiveContributionToSchema(
          updatedFormData,
          formSchema
        );
      } catch (e) {}
    ...
    fetch('/api/archive', {
      ...
      .then((res) => {
        if (!res.ok) {
          ...
          const emailContent = {
            contribution: 'Archive Contribution',
            id: data.archive.ID,
          };
